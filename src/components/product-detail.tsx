...
export function ProductDetail({ product }: { product: Product }) {
  ...
  return <main className="product-detail-page">
      ...
      <div className="product-detail-grid">
        <div className="detail-visual-wrap">
          <button type="button" className="detail-visual" style={{ "--product": product.color, "--accent": product.accent } as React.CSSProperties} onClick={() => setZoomed(true)} aria-label={`Zoom ${product.name} product image`}>
            ...
            <Image className="detail-product-photo" src={product.images[activeImage]} alt={`${product.brand} ${product.name}${activeImage > 0 ? ` product view ${activeImage + 1}` : ""}`} fill priority sizes="(max-width: 900px) 94vw, 52vw" />
          ...
          <div className="detail-thumbnails">{product.images.map((image, index) => <button key={image} className={activeImage === index ? "active" : ""} onClick={() => setActiveImage(index)} aria-label={`View product image ${index + 1}`}><Image src={image} alt="" fill sizes="78px"/></button>)}</div>
        </div>
    ...
    {zoomed && <div className="product-zoom-modal" role="dialog" aria-modal="true" aria-label={`${product.name} enlarged image`}><button className="zoom-close" onClick={() => setZoomed(false)} aria-label="Close enlarged image"><X/></button><div><Image src={product.images[activeImage]} alt={`${product.brand} ${product.name} enlarged`} fill sizes="100vw" /></div></div>}
